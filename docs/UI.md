...
| Server settings → Quality profiles | the default profile, and **full CRUD**: create, edit picture sizes and cutoff, reset a built-in, delete. The profiles are StingStream's own (`SharedSettings.QualityProfiles`), so all of it works before any indexer is set up; the sync copies them into both arrs |
