...
| `/stingstream/api/v1/identity/signin` | POST | Anonymous + a signed assertion in the **body**. The mesh verifies the Ed25519 signature against the issuer's node id (a node id *is* the public key, so nothing is exchanged beforehand), that `aud` is this node, and that it has not expired; Core then spends the nonce. A first arrival additionally needs a live invite — a genuine assertion from an unknown server proves identity and grants nothing, or every StingStream server would accept every other one's users. The account it creates gets the **derived password** that arrived with the assertion — `PBKDF2(password)`, made on the caller's own origin — so this server can check that password afterwards without ever having been told it. Absent (an older client), it falls back to a random password nobody knows |
| `/stingstream/api/v1/identity/signin-method` | POST | Anonymous, and it has to be: the caller is by definition somebody with no session here, and a client cannot derive its password without the salt. A salt is not a secret. **An ordinary account, a username nobody holds, and a linked account whose password has been reset all answer identically**, so this is not a way to enumerate accounts; what it tells somebody already holding a username is that it came from another server |
| `/stingstream/api/v1/identity/password` | POST | Session, and always the caller's own account — read from the session, never from the body. Takes the derived value and never the password. For somebody whose account came from another server and who has since changed their password there |
| `/stingstream/api/v1/identity/password/derivation/clear` | POST | Admin. Called by the Users screen straight after a password reset: Jellyfin now holds a hash of what was typed, and a client still deriving against the old salt would send something that cannot match — three of which lock the account |
...
| `/authorize` | Anyone. Serves the app through the SPA fallback; the request rides in the fragment and never reaches the server. It is the page on **your own** server that signs an assertion for somebody else's — which is why a redirect is used instead of a cross-origin call: the password stays on its own origin, and what comes back is a PBKDF2 of it rather than the password. That value is a credential for the one server that asked, and it does not expire, so `/join` drops the fragment out of the address bar before it does anything else |
