...
| Identity | Username + password on your **home node**, and — for somebody who accepted an invite with a server of their own — an account on that server too, whose password is a PBKDF2 of theirs derived before it left the device. Group membership is still node-to-node trust, and remote titles still appear in your own server's library. `docs/INVITES.md` §11. |
