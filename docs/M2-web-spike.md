...
| `@expo/react-native-action-sheet` | Works on web, but as a drawer sliding up from the bottom of the window, unlike every other web modal. | `lib/platform/web-stubs/expo-react-native-action-sheet.tsx` — the same API, each request handed to `lib/platform/actionSheetStore.ts` and drawn as a centred `Dialog` by `components/common/ActionSheetDialog.tsx`, mounted in the root layout. Phones keep the native sheet. |
