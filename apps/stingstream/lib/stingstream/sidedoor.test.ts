...
const OWN_HOST = "media.example.com";

function candidate(port = 443): SideDoorCandidate {
  const url =
    port === 443 ? `https://${OWN_HOST}` : `https://${OWN_HOST}:${port}`;
  return { kind: "own", host: OWN_HOST, port, url };
}
...
describe("diagnoseRebinding", () => {
  const outcome = (kind: "own" | "lan-ip-http", ok: boolean): ProbeOutcome => ({
    candidate:
      kind === "own"
        ? candidate()
        : {
  ...
  test("not enough evidence is reported as such rather than guessed at", () => {
    expect(diagnoseRebinding([outcome("own", false)]).rebinding).toBe(false);
...
describe("pickWinner", () => {
  ...
  test("the fastest encrypted candidate wins", () => {
    const w = pickWinner([mk("own", 10), mk("own", 2), mk("own", 40)]);
    expect(w?.candidate.kind).toBe("own");
  });
...
describe("probeCandidate", () => {
  test("a reply from the expected node is a win", async () => {
    const { impl } = fakeFetch({ [candidate().url]: {} });
    const out = await probeCandidate(candidate(), NODE, {
      fetchImpl: impl,
    });
  ...
  test("a reply from a different node is a failure, not a win", async () => {
    ...
    const { impl } = fakeFetch({
      [candidate().url]: { body: { node: "someothernode" } },
    });
    const out = await probeCandidate(candidate(), NODE, {
      fetchImpl: impl,
    });
  ...
  test("a non-200 is a failure carrying its status", async () => {
    const { impl } = fakeFetch({ [candidate().url]: { status: 503 } });
    const out = await probeCandidate(candidate(), NODE, {
      fetchImpl: impl,
    });
    expect(out.ok).toBe(false);
  ...
  test("it asks the CORS-safe endpoint and sends no credentials", async () => {
    ...
    await probeCandidate(candidate(), NODE, { fetchImpl: impl });
...
describe("raceSideDoor", () => {
  ...
  test("keeps the first candidate that answers", async () => {
    const { impl } = fakeFetch({
      [candidate().url]: { delay: 1 },
      [candidate(443).url]: { delay: 60 },
    });
  ...
  test("remembers the winner and tries it alone next time", async () => {
    ...
    const second = fakeFetch({ [candidate().url]: {} });
    const choice = await raceSideDoor(record(), {
  ...
  // The realistic case now that a node has one address: its owner changed their domain, so the
  // remembered winner points somewhere that no longer answers. Trying it, failing, and then racing
  // what the record actually says is the difference between a slow load and a broken one.
  test("a remembered winner that has stopped working re-races rather than failing", async () => {
    rememberWinner(
      NODE,
      { url: "https://old.example.com", kind: "own", secure: true, ms: 1 },
      store,
    );
    const { impl, calls } = fakeFetch({ [candidate().url]: {} });
    const choice = await raceSideDoor(record(), {
      fetchImpl: impl,
      store,
      timeoutMs: 30,
    });
    expect(choice?.url).toBe(candidate().url);
...
describe("remembering", () => {
  test("a winner is stored under the network it won on and as the last-known-good", () => {
    ...
    expect(recallWinner(NODE, store)?.kind).toBe("own");
    expect(
  ...
  test("an entry older than the TTL is discarded rather than trusted", () => {
    ...
    rememberWinner(
      NODE,
      { url: candidate().url, kind: "own", secure: true, ms: 1 },
      store,
      () => now,
    );
  ...
  test("a store that throws never propagates", () => {
    expect(() =>
      rememberWinner(
        NODE,
        { url: "x", kind: "own", secure: true, ms: 1 },
        throwingStore,
      ),
    ).not.toThrow();
...
describe("ownAddressRecord", () => {
  test("an https address becomes the one candidate to race", () => {
    const r = ownAddressRecord(NODE, "https://media.example.com");
  ...
  test("a port is carried through, because a self-hoster may not have 443", () => {
    const r = ownAddressRecord(NODE, "https://media.example.com:8443");
    expect(r?.candidates[0]).toMatchObject({
      port: 8443,
      url: "https://media.example.com:8443",
    });
  });

  test("a trailing slash does not become part of the origin", () => {
    expect(
      ownAddressRecord(NODE, "https://media.example.com/")?.candidates[0].url,
    ).toBe("https://media.example.com");
  });

  // The ordinary case, and the one that must not produce a candidate: a node with no domain is
  // reachable at home and through the app's mesh, and racing something invented would fail slowly
  // instead of saying so at once.
  test("no address is null, not an invented one", () => {
    expect(ownAddressRecord(NODE, null)).toBeNull();
    expect(ownAddressRecord(NODE, "")).toBeNull();
    expect(ownAddressRecord(NODE, "   ")).toBeNull();
  });

  // Plain http would be raced, would appear to work, and would then break secure storage and
  // crypto.randomUUID in the browser -- the exact crash this fork already hit on LAN origins.
  test("a plain-http address is refused rather than raced", () => {
    expect(ownAddressRecord(NODE, "http://media.example.com")).toBeNull();
  });

  test("junk is null rather than a plausible-looking wrong answer", () => {
    expect(ownAddressRecord(NODE, "not a url")).toBeNull();
  });
});
