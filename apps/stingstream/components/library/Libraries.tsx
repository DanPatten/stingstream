...
export const Libraries: React.FC = () => {
  ...
  // Widths in pixels, rather than percentages inside a cell whose own width is
  // the list's business: on Android a `width: "50%"` cell inside FlashList's
  // `numColumns` resolved against the *cell*, not the row, and the cards came
  // out a quarter of the size they should have been.
  //
  // And measured, not derived from the window: on a wide browser the web shell
  // puts a sidebar beside this page, so the window is a good deal wider than
  // the grid ever gets. `PageContainer` has already taken the page gutter off
  // what it hands down, so nothing is subtracted for it here.
  const rowWidth = measuredWidth - insets.left - insets.right;
  const cellWidth = rowWidth / columns;
  const cardWidth = Math.floor(
    (rowWidth - COLUMN_GAP * (columns - 1)) / columns,
  );
