...
//!
//! A StingStream node is one install and four processes behind one door. This crate is the door
//! and the thing that opens it:
//!
//! * [`supervisor`] — spawns, monitors, health-checks and restarts Jellyfin, Radarr and Sonarr,
//!   and pumps their output into structured JSON-lines logs.
...
//! * [`setup`] — whether first-run setup is still pending, and the one thing that depends on the
//!   answer: removing the generated bootstrap password once it is not.
