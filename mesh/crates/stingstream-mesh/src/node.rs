...
impl MeshNode {
    ...
    /// Where this node's account lives, and whether it has one yet.
    pub fn account_status(&self) -> Result<crate::api::AccountStatus> {
        let read = |key: &str| -> Result<Option<String>> {
            Ok(self.db.meta(key)?.filter(|v| !v.trim().is_empty()))
        };
        Ok(crate::api::AccountStatus {
            service: read(crate::accounts::SERVICE_KEY)?
                .or_else(|| crate::accounts::DEFAULT_ACCOUNT_SERVICE.map(str::to_string)),
            account: read(crate::accounts::ACCOUNT_KEY)?,
            username: read(crate::accounts::USERNAME_KEY)?,
            node: self.endpoint_id().to_z32(),
        })
    }

    /// Point this node at a different account service.
    ///
    /// Refused once the node is claimed. Moving a claimed server to another service would leave it
    /// holding an account id that service has never heard of, and the symptom would be sign-ins
    /// failing for a reason nothing on screen could explain.
    pub fn set_account_service(&self, url: Option<&str>) -> Result<crate::api::AccountStatus> {
        if self.db.meta(crate::accounts::ACCOUNT_KEY)?.is_some_and(|v| !v.is_empty()) {
            bail!("this server already belongs to an account; release it first");
        }
        let value = match url.map(str::trim).filter(|u| !u.is_empty()) {
            None => String::new(),
            Some(raw) => crate::accounts::normalize_service(raw)
                .ok_or_else(|| anyhow::anyhow!("that is not an address"))?,
        };
        self.db.set_meta(crate::accounts::SERVICE_KEY, &value)?;
        self.account_status()
    }

    /// Create an account, or attach this server to one that exists.
    ///
    /// Both go through the same door: this node signs, and the service checks the signature before
    /// anything else. `claim` additionally needs the account's password, because a signature alone
    /// would let anybody attach a machine they control to somebody else's account — and an attached
    ...
    pub async fn account_register(
        ...
        password: &str,
        claim_existing: bool,
    ) -> Result<crate::api::AccountStatus> {
        let status = self.account_status()?;
        let service = status
            .service
            .clone()
            .ok_or_else(|| anyhow::anyhow!("no account service is configured"))?;
        if status.account.is_some() {
            bail!("this server already belongs to an account");
        }

        let action = if claim_existing {
            crate::accounts::Action::Claim
        } else {
            crate::accounts::Action::Register
        };
        let body = crate::accounts::credential_body(username, password);
        let signed = crate::accounts::sign(&self.secret_key, action, &body, crate::util::now_unix());

        let path = if claim_existing { "claim" } else { "register" };
        let payload = serde_json::json!({
            ...
            "username": username,
            "password": password,
            "server_name": self.cfg.node_name,
        });

        let answer = self
            .account_post(&service, &format!("/accounts/v1/{path}"), &payload)
            .await?;
        let account = answer
            .get("account")
            .and_then(|v| v.as_str())
            .ok_or_else(|| anyhow::anyhow!("the account service did not return an account"))?;
        let name = answer
            .get("username")
            .and_then(|v| v.as_str())
            .unwrap_or(username);

        // The signing key comes down now rather than at the first sign-in: fetching it here means a
        // node that has been claimed can verify a token even if the service is unreachable ever
        // after, which is the property the whole design rests on.
        self.cache_account_key(&service).await?;

        self.db.set_meta(crate::accounts::SERVICE_KEY, &service)?;
        self.db.set_meta(crate::accounts::ACCOUNT_KEY, account)?;
        self.db.set_meta(crate::accounts::USERNAME_KEY, name)?;
        self.account_status()
    }

    /// Set a new password on the account this server belongs to.
    ///
    /// The only recovery there is: with no email there is no reset link, so proving you are the
    /// owner means proving you control a machine the account already owns.
    pub async fn account_reset(&self, password: &str) -> Result<crate::api::AccountStatus> {
        let status = self.account_status()?;
        let service = status
            .service
            .clone()
        ...
        if status.account.is_none() {
            bail!("this server does not belong to an account");
        }
        let body = crate::accounts::credential_body("", password);
        let signed = crate::accounts::sign(
            &self.secret_key,
            crate::accounts::Action::Reset,
            &body,
            crate::util::now_unix(),
        );
        self.account_post(
    ...
    /// Verify a token from the account service. **Local only** — see `crate::accounts`.
    pub fn account_verify(&self, token: &str) -> Result<crate::accounts::Claims> {
        let claims =
            crate::accounts::verify_token(&self.cfg.data_dir, token, crate::util::now_unix())?;
        // A token names the servers it may be presented to. One that does not name this node is a
        // token for somebody else's server, and honouring it would make every node interchangeable.
        let me = self.endpoint_id().to_z32();
        if !claims.servers.iter().any(|s| s == &me) {
            bail!("that token is not for this server");
        }
        Ok(claims)
    }

    async fn cache_account_key(&self, service: &str) -> Result<()> {
        let jwks: serde_json::Value = reqwest::Client::new()
            .get(format!("{service}/accounts/v1/jwks"))
            .timeout(std::time::Duration::from_secs(15))
            .send()
            .await
            .context("asking the account service for its signing key")?
            .json()
            .await
            .context("reading the account service's signing key")?;
        let z32 = jwks
            .get("keys")
            .and_then(|k| k.get(0))
            .and_then(|k| k.get("z32"))
            .and_then(|v| v.as_str())
            .ok_or_else(|| anyhow::anyhow!("the account service published no usable key"))?;
        crate::accounts::cache_key(&self.cfg.data_dir, z32)
    }
    ...
    async fn account_post(
        ...
        if !status.is_success() {
            // The service's own message is written to be read by a person — "that username is
            // taken" — so it is passed through rather than replaced with a status code.
