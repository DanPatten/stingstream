<#
.SYNOPSIS
    The "golden startup" acceptance harness for WP-TOOLS / v0.2.0: wipe, start, time every
...
.DESCRIPTION
    ...
    (runtime.json's first_run flag clears). The node starts on an EMPTY media root -- nothing
    seeds test content any more, so the library Home renders is whatever the machine actually
    holds. With -DriveUi, Playwright then opens the page, records first-contentful-paint, drives
    the first-run "Create your StingStream account" screen (creating the account and writing its
    credentials to a private --creds file, never runtime.json -- WP-CORE scrubs runtime.json's
    generated password once setup completes, so reading it back out for a later step is not
    reliable), and waits for an image with naturalWidth > 0 on Home (T_home). NOTE: with an empty
    library there may be no poster to wait for, so T_home now depends on Home rendering some
    image of its own (logo/empty-state art); if it times out, that is why. With -Lan, a second
    context hits the LAN URL and checks for
...
# ================================================================================================
Invoke-Step 'Wipe -> start (fresh data dir, mirrors a first run)' {
    ...
    # The media root has to exist before the node starts (first-run wiring points the Jellyfin
    # libraries at it), but nothing puts content in it -- see the .DESCRIPTION note about T_home.
