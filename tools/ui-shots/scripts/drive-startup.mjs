...
// firstrun-* testIDs), and a wait for Home to show a real poster. Prints one line,
// `UI_STARTUP_RESULT {json}`, which the calling PowerShell parses; everything else on stdout is
// just progress logging.
//
// F-36 follow-up (WP2, 2026-09-06): this used to read admin credentials out of runtime.json
// (--pass-file) and use them BOTH to sign in on a pre-WP3 build AND as the new account's password
// when the first-run screen showed. WP-CORE's setup renames the bootstrap admin and scrubs the
// generated password from runtime.json once setup completes -- reading it back out is only ever
// reliable before first-run finishes, which made the *later* step in ui-startup.ps1 (restart, then
// an ordinary login) throw every time. This drives first-run itself (creating its own account,
// same as shots.mjs --first-run) and writes the resulting {username,password} to --creds, so a
// later drive-login.mjs pass can sign back in without ever touching runtime.json. If the node
// somehow already has an account (the login screen shows instead of first-run -- not expected on
// the fresh data dir ui-startup.ps1 always starts from, but handled rather than assumed away),
// --creds must already exist and is read, not written.
//
//   node drive-startup.mjs --base http://127.0.0.1:8796 --out <dir> --creds <path to a
//     {username,password} JSON file> [--user stingstream] [--lan http://192.168.0.16:8796]

import path from "node:path";
import { chromium } from "playwright";
import { signIn, createFirstRunAccount } from "../flows/web.mjs";
import { readCreds, writeCreds } from "../lib/authFile.mjs";

function parseArgs(argv) {
  const args = { base: null, out: null, user: null, creds: null, lan: null };
  ...
  if (!args.base || !args.out || !args.creds) {
    throw new Error("usage: drive-startup.mjs --base <url> --out <dir> --creds <path to {username,password} JSON> [--user <name>] [--lan <url>]");
  }
  return args;
}
...
async function main() {
  ...
  try {
    ...
    const hasSetupScreen = await page
      ...
      .catch(() => false);
    if (hasSetupScreen) {
      console.log("first-run setup screen found (firstrun-username)");
      const tSetup = Date.now();
      await page.screenshot({ path: path.join(args.out, "startup-02-setup.png") });
      const created = await createFirstRunAccount(page, { base: args.base, username: args.user || undefined });
      writeCreds(args.creds, created);
      result.setupSeconds = (Date.now() - tSetup) / 1000;
      await page.screenshot({ path: path.join(args.out, "startup-03-submitted.png") });
    } else {
      console.log("no first-run setup screen -- this data dir already has an account; signing in with --creds");
